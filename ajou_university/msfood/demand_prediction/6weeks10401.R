library(tseries)
library(forecast)
mstimelog104<-read.csv("c:/Users/Data1/Desktop/6주차2/mstime104log.csv")
rownames(mstimelog104)<-as.Date(mstimelog104$X)
mstimelog104<-mstimelog104[,-1]
stat104<-read.csv("c:/Users/Data1/Desktop/6주차2/stat104.csv",stringsAsFactors = F)
rmsedivmean<-read.csv("c:/Users/Data1/Desktop/6주차2/rmsedifvmean05?씠?븯.csv",stringsAsFactors = F)
...
fit1<-as.data.frame(matrix(rep(NA,96824),ncol = 931))
predict1<-as.data.frame(matrix(rep(NA,3724),ncol=931))
colnames(predict1)<-rmsedivmean$item_code
colnames(fit1)<-rmsedivmean$item_code

setwd("c:/Users/Data1/Desktop/6二쇱감2/104/")
for (t1 in rmsedivmean$item_code[1:10]) {
  rmse=NULL
    for(t2 in list1){
    tryCatch({
      model = arima(mstimelog104[t1],order = c(t2[1],t2[2],t2[3]), seasonal = list(order=c(t2[4],t2[5],t2[6]),period=16))
    }, error =function(e){cat("ERROR :", conditionMessage(e), "\n")})
    rmse = append(rmse,accuracy(model)[2])
    }
  min_index = which.min(rmse)
  min_order = as.vector(list1[min_index])
  model2<-arima(mstimelog104[t1],order = c(min_order[[1]][1],min_order[[1]][2],min_order[[1]][3]),seasonal = list(order=c(min_order[[1]][4],min_order[[1]][5],min_order[[1]][6]),period=16))
...
#03
mstimelog104diff<-diff(mstimelog104)
unit2<-NULL
...
stat104$dif[is.na(stat104$dif)]<-0
...
stat104$unit2[stat104$item_code=="X107804"]

setwd("c:/Users/Data1/Desktop/6주차2/104/")
rmsedivmean$item_code[1:10]
for (t1 in rmsedivmean$item_code[1]) {
  rmse=NULL
  for(t2 in list1){
    tryCatch({
